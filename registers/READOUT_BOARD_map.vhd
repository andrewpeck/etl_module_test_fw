...
architecture behavioral of READOUT_BOARD_wb_interface is
  ...
  signal reg_data :  slv32_array_t(integer range 0 to 73);
  constant DEFAULT_REG_DATA : slv32_array_t(integer range 0 to 73) := (others => x"00000000");
begin  -- architecture behavioral
  ...
  begin  -- process reads
    if rising_edge(clk) then  -- rising clock edge
      ...
      if wb_strobe='1' then
        ...
        when 69 => --0x45
          localRdData( 0)            <=  reg_data(69)( 0);                          --Enable flag to select SCAs
        when 53 => --0x35
        ...
        when 57 => --0x39
          localRdData( 7 downto  0)  <=  reg_data(57)( 7 downto  0);                --Command: The Command field is present in the frames received by the SCA and indicates the operation to be performed. Meaning is specific to the channel.
          localRdData(15 downto  8)  <=  reg_data(57)(15 downto  8);                --Command: It represents the packet destination address. The address is one-byte long. By default, the GBT-SCA use address 0x00.
          localRdData(23 downto 16)  <=  reg_data(57)(23 downto 16);                --Command: Specifies the message identification number. The reply messages generated by the SCA have the same transaction identifier of the request message allowing to associate the transmitted commands with the corresponding replies, permitting the concurrent use of all the SCA channels.  It is not required that ID values are ordered. ID values 0x00 and 0xff are reserved for interrupt packets generated spontaneously by the SCA and should not be used in requests.
          localRdData(31 downto 24)  <=  reg_data(57)(31 downto 24);                --Command: The channel field specifies the destination interface of the request message (ctrl/spi/gpio/i2c/jtag/adc/dac).
        when 51 => --0x33
          localRdData(15 downto  0)  <=  reg_data(51)(15 downto  0);                --Address of the first register to be accessed
        when 52 => --0x34
          localRdData(15 downto  0)  <=  reg_data(52)(15 downto  0);                --Number of words/bytes to be read (only for read transactions)
        when 58 => --0x3a
          localRdData(31 downto  0)  <=  reg_data(58)(31 downto  0);                --Command: data field (According to the SCA manual)
        when 59 => --0x3b
  ...
  Ctrl.SC.TX_START_WRITE        <=  reg_data(50)( 0);               
  Ctrl.SC.TX_START_READ         <=  reg_data(50)( 1);               
  Ctrl.SC.SCA_ENABLE            <=  reg_data(69)( 0);               
  Ctrl.SC.TX_DATA_TO_GBTX       <=  reg_data(53)( 7 downto  0);     
  Ctrl.SC.TX_CMD                <=  reg_data(57)( 7 downto  0);     
  Ctrl.SC.TX_GBTX_ADDR          <=  reg_data(50)(15 downto  8);     
  Ctrl.SC.TX_ADDRESS            <=  reg_data(57)(15 downto  8);     
  Ctrl.SC.TX_REGISTER_ADDR      <=  reg_data(51)(15 downto  0);     
  Ctrl.SC.TX_NUM_BYTES_TO_READ  <=  reg_data(52)(15 downto  0);     
  Ctrl.SC.TX_TRANSID            <=  reg_data(57)(23 downto 16);     
  Ctrl.SC.TX_CHANNEL            <=  reg_data(57)(31 downto 24);     
  Ctrl.SC.TX_DATA               <=  reg_data(58)(31 downto  0);     
  ...
  begin  -- process reg_writes
    if (rising_edge(clk)) then  -- rising clock edge
      ...
      if wb_strobe='1' and wb_write = '1' then
        ...
        when 69 => --0x45
          reg_data(69)( 0)                 <=  localWrData( 0);                --Enable flag to select SCAs
        when 70 => --0x46
        ...
        when 57 => --0x39
          reg_data(57)( 7 downto  0)       <=  localWrData( 7 downto  0);      --Command: The Command field is present in the frames received by the SCA and indicates the operation to be performed. Meaning is specific to the channel.
          reg_data(57)(15 downto  8)       <=  localWrData(15 downto  8);      --Command: It represents the packet destination address. The address is one-byte long. By default, the GBT-SCA use address 0x00.
          reg_data(57)(23 downto 16)       <=  localWrData(23 downto 16);      --Command: Specifies the message identification number. The reply messages generated by the SCA have the same transaction identifier of the request message allowing to associate the transmitted commands with the corresponding replies, permitting the concurrent use of all the SCA channels.  It is not required that ID values are ordered. ID values 0x00 and 0xff are reserved for interrupt packets generated spontaneously by the SCA and should not be used in requests.
          reg_data(57)(31 downto 24)       <=  localWrData(31 downto 24);      --Command: The channel field specifies the destination interface of the request message (ctrl/spi/gpio/i2c/jtag/adc/dac).
        when 51 => --0x33
          reg_data(51)(15 downto  0)       <=  localWrData(15 downto  0);      --Address of the first register to be accessed
        when 52 => --0x34
          reg_data(52)(15 downto  0)       <=  localWrData(15 downto  0);      --Number of words/bytes to be read (only for read transactions)
        when 58 => --0x3a
          reg_data(58)(31 downto  0)       <=  localWrData(31 downto  0);      --Command: data field (According to the SCA manual)
      ...
      if reset = '1' then
      reg_data(50)( 0)  <= DEFAULT_READOUT_BOARD_CTRL_t.SC.TX_START_WRITE;
      reg_data(50)( 1)  <= DEFAULT_READOUT_BOARD_CTRL_t.SC.TX_START_READ;
      reg_data(69)( 0)  <= DEFAULT_READOUT_BOARD_CTRL_t.SC.SCA_ENABLE;
      reg_data(53)( 7 downto  0)  <= DEFAULT_READOUT_BOARD_CTRL_t.SC.TX_DATA_TO_GBTX;
      reg_data(57)( 7 downto  0)  <= DEFAULT_READOUT_BOARD_CTRL_t.SC.TX_CMD;
      reg_data(50)(15 downto  8)  <= DEFAULT_READOUT_BOARD_CTRL_t.SC.TX_GBTX_ADDR;
      reg_data(57)(15 downto  8)  <= DEFAULT_READOUT_BOARD_CTRL_t.SC.TX_ADDRESS;
      reg_data(51)(15 downto  0)  <= DEFAULT_READOUT_BOARD_CTRL_t.SC.TX_REGISTER_ADDR;
      reg_data(52)(15 downto  0)  <= DEFAULT_READOUT_BOARD_CTRL_t.SC.TX_NUM_BYTES_TO_READ;
      reg_data(57)(23 downto 16)  <= DEFAULT_READOUT_BOARD_CTRL_t.SC.TX_TRANSID;
      reg_data(57)(31 downto 24)  <= DEFAULT_READOUT_BOARD_CTRL_t.SC.TX_CHANNEL;
      reg_data(58)(31 downto  0)  <= DEFAULT_READOUT_BOARD_CTRL_t.SC.TX_DATA;
